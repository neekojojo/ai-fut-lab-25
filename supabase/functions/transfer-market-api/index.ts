...
serve(async (req) => {
  console.log("⚽ استدعاء Transfer Market API Edge Function");
  ...
  try {
    ...
    const API_KEY = Deno.env.get('TRANSFER_MARKET_API_KEY');
    const API_BASE_URL = Deno.env.get('TRANSFER_MARKET_API_BASE_URL') || 'https://api.transfermarkt.com/v1';
    
    if (!API_KEY) {
      console.error("❌ مفتاح API غير موجود: TRANSFER_MARKET_API_KEY");
      throw new Error('TRANSFER_MARKET_API_KEY is not set in environment variables');
    }
    ...
    console.error('❌ خطأ في Transfer Market API proxy:', error);
