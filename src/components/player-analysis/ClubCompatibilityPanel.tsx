...
const ClubCompatibilityPanel: React.FC<ClubCompatibilityPanelProps> = ({ playerAnalysis }) => {
        ...
        <div className="space-y-4">
          {compatibilityResults.map((result) => (
            <Card key={result.team.id} className={`overflow-hidden border ${
              result.compatibilityScore > 80 ? 'border-green-200 bg-green-50' : 
              result.compatibilityScore > 60 ? 'border-blue-200 bg-blue-50' : 
              'border-gray-200'
            }`}>
              <div className="p-4">
...
export default ClubCompatibilityPanel;
