...
interface PlayerHeatMapViewProps {
  heatmapData?: Array<{x: number; y: number; intensity: number}>;
}

const PlayerHeatMapView: React.FC<PlayerHeatMapViewProps> = ({ 
  heatmapData = [
      ...
      <CardContent>
        <Tabs defaultValue="first-half">
          <TabsList className="grid w-full grid-cols-2 mb-4">
            <TabsTrigger value="first-half">الشوط الأول</TabsTrigger>
            <TabsTrigger value="second-half">الشوط الثاني</TabsTrigger>
          </TabsList>
          
          <TabsContent value="first-half">
          ...
          <TabsContent value="second-half">
            <div className="relative h-[280px] w-full bg-gradient-to-b from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 rounded-lg overflow-hidden">
              ...
              {/* رسم نقاط الحرارة مختلفة للشوط الثاني */}
              {heatmapData.map((point, index) => (
                <div 
                  key={index}
                  className="absolute rounded-full bg-red-500/80"
                  style={{
                    left: `${100 - point.x * 0.7}%`,
                    top: `${point.y * 0.9}%`,
                    width: `${Math.max(2, point.intensity * 25)}px`,
                    height: `${Math.max(2, point.intensity * 25)}px`,
                    transform: 'translate(-50%, -50%)',
                    opacity: Math.max(0.2, point.intensity * 0.8),
                  }}
                />
              ))}
...
export default PlayerHeatMapView;
